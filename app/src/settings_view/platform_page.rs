...
impl PlatformPageView {
    ...
    pub fn new(ctx: &mut ViewContext<PlatformPageView>) -> Self {
        let api_key_search_editor = ctx.add_typed_action_view(|ctx| {
            ...
            editor.set_placeholder_text("搜索 API 密钥", ctx);
            editor
        });
    ...
    fn handle_create_api_key_modal_event(
        ...
        match event {
            ...
            CreateApiKeyModalEvent::Created { api_key } => {
                self.create_api_key_modal_state
                    .set_title(Some("保存你的密钥".to_string()), ctx);
...
impl PlatformPageWidget {
    fn render_description_with_link(
        ...
        let text = vec![
            FormattedTextFragment::plain_text("创建和管理 API 密钥，以便其他 Oz Cloud 智能体访问你的 Warp 账户。\n如需了解更多，请查阅"),
            ...
            Flex::row()
                ...
                .with_child(
                    ui_builder
                        ...
                        .with_text_label("+ 新建 API 密钥".to_string())
                        .build()
                        .on_click(|ctx, _, _| {
    ...
    fn render_api_keys_header(
        ...
        header_row.add_child(
            ConstrainedBox::new(self.render_header_cell(appearance, "密钥"))
